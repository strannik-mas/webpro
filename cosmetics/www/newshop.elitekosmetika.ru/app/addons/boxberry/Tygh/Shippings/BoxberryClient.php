...
class BoxberryClient extends Client
{
    ...
    {
        if (empty($this->key)) {
            throw new BadSettingsException(__('boxberry.enter_api_token'));
        }

        if ($request->checkRequiredFields() === false) {
            throw new RequiredFieldsNullException(__('boxberry.not_all_fields_are_filled'));
        }
        ...
        if (!empty($answer['err'])) {
            ...
            throw new UnknownTypeException(__('boxberry.unknown_error'));
        }
    }
    ...
    public function getStatus($tracking_number)
    {
        ...
        try {
            ...
            $status = __('boxberry.without_status');
        }
     ...
     */
    public function getPickupPoints($city_name)
    {
