...
function fn_yml_export_update_product_pre(&$product_data, $product_id, $lang_code, $can_update)
{
    ...
    if (isset($product_data['yml2_exclude_price_ids'])) {
        ...
        $old_price_ids = db_get_fields(
            "SELECT price_id FROM ?:yml_exclude_objects WHERE object_id = ?i AND object_type = 'product'",
            $product_id
        );
        ...
        if (!empty($delete_prices_ids)) {
            db_query(
                "DELETE FROM ?:yml_exclude_objects WHERE price_id IN (?a) AND object_type = 'product'",
                $delete_prices_ids
            );
        }
    }
...
function fn_yml_get_parent_categories_field($field_name, $category_id, $parent = false)
{
    ...
    if (!isset($paths[$category_id])) {
        $paths[$category_id] = db_get_field('SELECT id_path FROM ?:categories WHERE category_id = ?i', $category_id);
    }
...
function fn_yml_export_import_ym($params)
{
    ...
    if (!empty($params['import_settings']) && $params['import_settings'] == 'Y') {
        $price_ids = fn_yml_export_import_settings();
        $import_success = true;
    }
    ...
    if ($import_success) {
        fn_set_notification('N', __('notice'), __('yml_export.import_success'));
    } else {
        fn_set_notification('W', __('warning'), __('yml_export.import_failed'));
    }
}

function fn_yml_export_import_products($price_ids)
{
    ...
    $products = db_get_fields('SELECT product_id FROM ?:products WHERE yml_export_yes = ?s', 'N');
    $exclude_object = array();
    foreach ($price_ids as $company_id => $price_id) {
    ...
    if (!empty($exclude_object)) {
        db_query('INSERT INTO ?:yml_exclude_objects ?m', $exclude_object);
    }
}

function fn_yml_export_import_categories($price_ids)
{
    db_query("UPDATE ?:categories SET yml2_market_category = yml_market_category");
    $price_lists = fn_yml_get_price_lists();

    foreach ($price_ids as $company_id => $price_id) {
        $categories = db_get_fields('SELECT category_id FROM ?:categories WHERE yml_disable_cat = ?s AND company_id = ?i', 'Y', $company_id);

        $price = $price_lists[$price_id]['param_data'];
        $price['exclude_categories'] = implode(',', $categories);

        fn_yml_update_price_list($price_id, $price);
    }
}
...
function fn_yml_export_import_settings()
{
    ...
    foreach($companies_ids as $company_id) {
        ...
        $disabled_categories = db_get_fields("SELECT category_id FROM ?:categories WHERE yml_disable_cat = 'Y'");
        $price_params = array(
            'enable_authorization' => fn_yml_get_setting_value('enable_authorization', $company_id),
            'company_id' => $company_id,
            'access_key' => $key,
            'name_price_list' => 'Яндекс.Маркет (' . __('import') . ')',
            'shop_name' => $shop_name,
            ...
            'export_market_category' => fn_yml_get_setting_value('market_category', $company_id),
            'weight' => 'N',
            'dimensions' => 'N',
            ...
            'export_hidden_categories' => fn_yml_get_setting_value('disable_cat_d', $company_id, true),
            'exclude_categories_not_logging' => 'Y',
            'exclude_categories' => implode(',', $disabled_categories),
        );

        fn_yml_get_categories_data($price_params);

        $price_ids[$company_id] = fn_yml_update_price_list(0, $price_params);
    }
