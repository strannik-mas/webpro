...
function fn_seo_get_product_data_post(&$product_data, &$auth, &$preview, &$lang_code)
{
    ...
    $amount = 0;
    if (!empty($product_data['inventory_amount'])) {
        $amount = $product_data['inventory_amount'];
    } elseif (!empty($product_data['amount'])) {
    ...
    if (count($languages) > 1) {
        foreach ($languages as $language) {
            if ($language['lang_code'] == $default_language) {
                $seo_alt_hreflangs_list['x-default'] = $seo_alt_hreflangs_list[$language['lang_code']] =
                    array(
                        'name' => $language['name'],
                        'direction' => $language['direction'],
                        'href' => fn_query_remove(fn_url(Registry::get('config.current_url')), $lang_parameter)
                    );
            } else {
                $seo_alt_hreflangs_list[$language['lang_code']] =
                    array(
                        'name' => $language['name'],
                        'direction' => $language['direction'],
                        'href' => fn_link_attach(fn_url(Registry::get('config.current_url')), "{$lang_parameter}={$language['lang_code']}")
                    );
            }
        }
    }
