...
$group_key = $shipping_info['keys']['group_key'];
  ...
  private function sendRequest($xml)
  {
        ...
        if ($this->_shipping_info['service_code'] == 'shoplogistics_pickup')
          {
               ...
               $currency = db_get_row("SELECT * FROM ?:currencies WHERE currency_code=?s", "RUB");

               if ($currency['is_primary']=="N"){

                   $rates['price']=$rates['price']*$currency['coefficient'];

               }

               ///cs-commerce mods end
        ...
        else
          {
            ...
            if ($currency['is_primary']=="N"){
              ...
              $price= $price*$currency['coefficient'];
   ...
   ///cs-commerce mods end
            ...
            $return = array(
                    'cost' => $price,
                    'delivery_time' => $ar[count($ar)-1] ." ".__('days'), //cs-commerce mod
                    'error' => true,
                );
            return $return;
          }
