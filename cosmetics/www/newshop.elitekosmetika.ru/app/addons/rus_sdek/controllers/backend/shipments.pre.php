...
if ($mode == 'details') {
    if (!empty($_REQUEST['shipment_id'])) {
        ...
        $data_call_recipients = db_get_array("SELECT * FROM ?:rus_sdek_call_recipient WHERE order_id = ?i and shipment_id = ?i", $shipment['order_id'], $shipment['shipment_id']);
        ...
        $status = array();
        $data_status = db_get_row("SELECT status, city_code, timestamp FROM ?:rus_sdek_history_status WHERE order_id = ?i AND shipment_id = ?i ORDER BY timestamp DESC", $shipment['order_id'], $shipment['shipment_id']);

        if (!empty($data_status)) {
            $status = $data_status;
            $status['city'] = db_get_field("SELECT city FROM ?:rus_city_descriptions as a LEFT JOIN ?:rus_cities as b ON a.city_id=b.city_id WHERE b.sdek_city_code = ?s", $data_status['city_code']);
        }
