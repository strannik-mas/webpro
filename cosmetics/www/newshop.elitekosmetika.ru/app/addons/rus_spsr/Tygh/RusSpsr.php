...
EOT;
            ...
            if (!empty($xml->Result['RC'])) {
                ...
                if ($status_code != 0) {
                    ...
                    self::$sid = (!empty($xml->Login['SID'])) ? (string) $xml->Login['SID'] : '';
                    $return = true;
                }
            }

            if (!empty($xml->Login['SID'])) {
    ...
    public static function deliveryTariffs($invoice, $order_info, $shipping_data)
    {
        ...
        if (isset($xml->Error)) {
            ...
            if ($xml->Tariff->Total_Dost == 'Error') {
                fn_set_notification('E', __('notice'), (string)$xml->Tariff->TariffType);
            } else {
                $_result = array();
