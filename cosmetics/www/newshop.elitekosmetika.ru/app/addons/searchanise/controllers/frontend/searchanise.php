...
if ($mode == 'info') {
    ...
    if (empty($_REQUEST['parent_private_key']) || fn_se_get_parent_private_key($company_id, DEFAULT_LANGUAGE) !== $_REQUEST['parent_private_key']) {
        ...
        if (isset($_REQUEST['product_id'])) {
            ...
            $options = fn_se_get_products_data(array($_REQUEST['product_id']), $company_id, $lang_code, false);
...
function fn_se_get_products_data($product_ids, $company_id = 0, $lang_code = NULL, $fl_echo = true)
{
    ...
    if (!empty($product_ids)) {
        list($products) = fn_get_products(array(
            ...
            'sort_by' => 'null',
            'pid'     => $product_ids,
            'extend'  => array('description', 'search_words', 'popularity', 'sales', 'discount'),
        ), 0, $lang_code);
    }
...
function fn_se_get_products_additionals(&$products, $company_id, $lang_code)
{
    ...
    foreach ($products as &$product) {
        ...
        if (fn_allowed_for('MULTIVENDOR')) {
            $product['active_company'] = in_array($product['company_id'], $company_ids)? 'Y' : 'N';
        } else {
            $product['active_company'] = 'Y';
        }
...
function fn_se_prepare_product_data($product_data, $usergroups, $company_id, $lang_code)
{
    ...
    $entry = array(
        ...
        'list_price'  => array(
            'value' => fn_format_price($product_data['list_price']),
    ...
    //
    //
    if ($product_data['free_shipping'] == 'Y') {
        $entry['free_shipping'] = array(
            'name'  => 'free_shipping',
     ...
     */
    if (!empty($product_data['approved']) && in_array($product_data['approved'], array('Y', 'N'))) {
