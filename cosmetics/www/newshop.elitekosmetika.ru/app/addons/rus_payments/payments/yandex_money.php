...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    $post_data = array(
        ...
        'shopDefaultUrl' => fn_url("payment_notification.return?payment=yandex_money&ordernumber=$orderNumber", AREA, 'https'),
        ...
        'paymentType' => !empty($payment_info['yandex_payment_type']) ? mb_strtoupper($payment_info['yandex_payment_type']) : '',
        'cms_name' => 'cscart'
    );
    ...
    if (fn_is_yandex_checkpoint_receipt_required($processor_data)) {
        ...
        if ($receipt) {
            $post_data['ym_merchant_receipt'] = json_encode($receipt);
        }
    }
    ...
    fn_create_payment_form($post_address, $post_data, $processor_data['processor'], false);
}
