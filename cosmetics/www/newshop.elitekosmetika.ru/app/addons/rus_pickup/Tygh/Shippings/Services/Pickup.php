...
class Pickup implements IService
{
    ...
    private function _checkStoreCost($stores)
    {
        ...
        if ($check) {
            $select_store = reset($stores);
            $min_surcharge = $select_store['pickup_surcharge'];

            foreach ($stores as $store) {
                if ($store['pickup_surcharge'] < $min_surcharge) {
                    $min_surcharge = $store['pickup_surcharge'];
                    $select_store = $store;
                }
            }

            $pickup_surcharge = $select_store['pickup_surcharge'];
            $_SESSION['cart']['select_store'][$group_key][$shipping_id] = $select_store['store_location_id'];
