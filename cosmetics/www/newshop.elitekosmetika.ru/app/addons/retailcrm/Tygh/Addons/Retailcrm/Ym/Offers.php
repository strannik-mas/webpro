...
class Offers
{
    ...
    public function getOffer($product)
    {
        ...
        if (!isset($this->offers[$offer_type])) {
            ...
            $offer_class = "\\Tygh\\Addons\\Retailcrm\\Ym\\Offers\\" . fn_camelize($offer_type);

            if (class_exists($offer_class)) {
                $offer = $this->offers[$offer_type] = new $offer_class($this->options, $this->log);
            } elseif (class_exists($base_offer_class)) {
                $offer = $this->offers[$offer_type] = new $base_offer_class($this->options, $this->log);
            } else {
