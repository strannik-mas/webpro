...
class Api extends YandexObject
{
    ...
    public function getGeoID($package_info)
    {
        ...
        $geo_id = db_get_field(
            'SELECT yd_geo_id FROM ?:yd_geo WHERE yd_state = ?s AND yd_city = ?s',
            $package_info['location']['state'], $city
        );

        if (empty($geo_id)) {
            if (!empty($package_info['location']['state_descr'])) {
                $state_descr = $package_info['location']['state_descr'];
            } else {
                $state_descr = fn_get_state_name($package_info['location']['state'], $package_info['location']['country']);
            }

            $address = $state_descr . ', ' . $city;
