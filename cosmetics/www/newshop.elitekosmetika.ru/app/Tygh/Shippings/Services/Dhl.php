...
EOT;
     ...
     */
    private function getShipmentRate(\Node $shipment, array $currencies, $primary_currency)
    {
        $rates_list[$shipment->getValueByPath('CurrencyCode')] =
            (float) $shipment->getValueByPath('ShippingCharge');
        ...
        // check if any of specified currencies exists in the store
        foreach ($rates_list as $currency_code => $rate) {
            if ($currency_code == $primary_currency) {
                return $rate;
            } elseif (!empty($currencies[$currency_code])) {
                // convert to primary
