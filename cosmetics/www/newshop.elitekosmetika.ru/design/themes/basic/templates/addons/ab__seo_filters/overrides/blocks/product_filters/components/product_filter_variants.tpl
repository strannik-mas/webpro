...
{if !$variant.disabled and $href|strpos:"features_hash" === false and !empty($href)}<a class="cm-ajax cm-ajax-full-render cm-history" data-ca-target-id="product_filters_*,products_search_*,category_products_*,product_features_*,breadcrumbs_*,currencies_*,languages_*,selected_filters_*" href="{$href}">{/if}
