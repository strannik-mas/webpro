<div class="ty-discussion-post" id="new_post_dialog_{$obj_prefix}{$obj_id}" title="{$new_post_title}">
<h2 class="ty-discussion-post-title">Оставить отзыв</h2>
...
<div class="ty-control-group">
    <label style="display: none;" for="dsc_name_{$obj_prefix}{$obj_id}" class="ty-control-group__title cm-required">{__("your_name")}</label>
    <input type="text" placeholder="{__("your_name")}" id="dsc_name_{$obj_prefix}{$obj_id}" name="post_data[name]" value="{if $auth.user_id}{$user_info.firstname} {$user_info.lastname}{elseif $discussion.post_data.name}{$discussion.post_data.name}{/if}" size="50" class="ty-input-text-large" />
</div>
...
<div class="ty-control-group">
    <label style="display: none;" for="dsc_message_{$obj_prefix}{$obj_id}" class="ty-control-group__title cm-required">{__("your_message")}</label>
    <textarea placeholder="{__("your_message")}" id="dsc_message_{$obj_prefix}{$obj_id}" name="post_data[message]" class="ty-input-textarea ty-input-text-large" rows="5" cols="72">{$discussion.post_data.message}</textarea>
</div>
{*/if*}
{/hook}
