{capture name="extra"}
    ...
    <div class="pull-left">
        {if $product_options}
            ...
            {if $product_data.product_type === "\Tygh\Addons\ProductVariations\Product\Manager::PRODUCT_TYPE_CONFIGURABLE"|constant}
                {include file="buttons/button.tpl" but_text=__("product_variations.btn_generate") but_href="product_variations.generate?product_id=`$product_data.product_id`" but_meta="btn"  but_role="text"}
            {else}
                {if $has_inventory}
