...
<form action="{""|fn_url}" method="post" enctype="multipart/form-data" class="form-horizontal form-edit cm-skip-check-items {if $dynamic_object}cm-hide-inputs{/if}  {if $smarty.request.ajax_update}cm-ajax cm-form-dialog-closer{/if}" name="block_{$id}_update_form">
