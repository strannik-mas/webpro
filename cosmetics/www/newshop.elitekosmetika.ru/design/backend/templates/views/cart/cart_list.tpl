...
<tr>
    ...
    <th width="20%">
        <span id="off_carts" alt="{__("expand_collapse_list")}" title="{__("expand_collapse_list")}" class="hidden hand cm-combinations-carts"/><span class="icon-caret-down"></span></span>
        <span id="on_carts" alt="{__("expand_collapse_list")}" title="{__("expand_collapse_list")}" class="cm-combinations-carts"><span class="icon-caret-right"></span></span>
...
<tr>
    ...
    <td>
        {if "ULTIMATE"|fn_allowed_for}
            ...
            <span alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_user_{$customer.user_id}_{$customer.company_id}" class="hidden cm-combination-carts"><span class="icon-caret-down"></span></span>
        {/if}
        ...
        {if !"ULTIMATE"|fn_allowed_for}
            ...
            <span alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_user_{$customer.user_id}" class="hidden cm-combination-carts"><span class="icon-caret-down"></span></span>
        {/if}

        {if $customer.user_data.email}<a href="{"profiles.update?user_id=`$customer.user_id`"|fn_url}" class="underlined">{if $customer.firstname || $customer.lastname}{$customer.lastname} {$customer.firstname}{else}{$customer.user_data.email}{/if}</a>{if $customer.phone}<br />{__('phone')} {$customer.phone}{/if}{else}{__("unregistered_customer")} {if $customer.lastname || $customer.firstname}<p>{$customer.lastname} {$customer.firstname} {if $customer.email}({$customer.email}){/if}{if $customer.phone}<br />{__('phone')} {$customer.phone}{/if}{/if}</p>{/if}
    ...
    <td>{if $customer.order_id}<a href="{"orders.details?order_id=`$customer.order_id`"|fn_url}">{$customer.order_id}</a>{/if}</td>
