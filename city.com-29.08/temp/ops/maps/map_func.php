...
function getAllLabels($type) {
	global $link;
	if (!empty($type)) {
		$sql = "SELECT t1.lat, t1.long, t1.desc, t1.title, t1.user, t1.username, `marker_types`.`typename`, `images`.`path` as marker_path, `images`.`filename` as marker_filename, t1.date, t1.uid
		...
		if ($type[0] == 'AllDiv') {//если на странице архива меток
			$sql = "SELECT t1.lat, t1.long, t1.desc, t1.title, t1.user, t1.username, `marker_types`.`typename`, `images`.`path` as marker_path, `images`.`filename` as marker_filename, t1.date, t1.uid, COUNT(  `comments`.`entid` )
							FROM googlemaplabels t1
							LEFT JOIN marker_types ON marker_types.id = t1.type
							LEFT JOIN images ON images.type = t1.type AND images.entity = 'marker'
...
<div id="record_$id" class="record $class">
	<a class="avatar" href="index.php?ops=users2&type=page&uid=$uuid"><img src="$avatar" alt="avatar" /></a>
...
function similar_labels() {
	global $link;
	$uid1 = $_GET['item'];
	$lon2 = elem_ad($uid1,'long', 'googlemaplabels');
	$lat2 = elem_ad($uid1,'lat', 'googlemaplabels');

	// $where = " AND city LIKE '%{$city}%' ";
	$where = " AND uid NOT LIKE '%{$uid1}%' ";

	$query = "SELECT * FROM googlemaplabels WHERE 1 {$where} ORDER BY `id` DESC LIMIT 1";
	$labels = $link -> query($query);
	while ($label = mysqli_fetch_assoc($labels)) {
		$id = $label['id'];
		$uid = $label['uid'];
...
<h5>{$label['title']}</h5>
<p class="type"><span>Тип: </span>$type</p>
<p class="type"><span>Адрес: </span>$laddress</p>
<div class="text-center"><a href="index.php?ops=maps&type=archive&item={$uid}" class="more">Подробнее</a></div>
EOT;
		// }
}
}
