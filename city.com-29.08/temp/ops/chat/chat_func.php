...
function list_chat() {
	global $link;
	$city = strlen($_COOKIE['chatcity']) > 0 ? $_COOKIE['chatcity'] : data_user('city');
	$query = "SELECT t.* FROM (SELECT * FROM `chat` WHERE `city` LIKE '%{$city}%' ORDER BY `chat`.id DESC LIMIT 200) t ORDER BY t.id ASC";
...
<div id="record_$id" class="record $class">
	...
	<div class="reply" onclick="$('#parent').val('{$id}'); $('.message-function h2').html('ответить');">Reply</div>
	<a class="sms" onclick="$('#minichatform .dropdown-menu').toggle(); newrequest('getMiniChat','#scrollbarminichat','$uuid'); newrequest('cookie','','chatto,$uuid,3600');"><i class="fa fa-envelope" aria-hidden="true"></i></a>
</div>
$answer
EOT;
	}
...
function add_help() {
	...
	if (strlen($message)) {
		...
		if ($parent > 0) {
			$link -> query("INSERT INTO `notifications`(`unit`, `type`, `from`, `message`, `status`) VALUES ('helpchat', 'message', '{$uid}', '{$message}', 0)") or die($link -> error);
		}
	}
	echo list_help();
}
...
function list_help() {
	global $link;
	$city = strlen($_COOKIE['chatcity']) > 0 ? $_COOKIE['chatcity'] : data_user('city');
	$query = "SELECT t.* FROM (SELECT * FROM `helpchat` WHERE `city` LIKE '%{$city}%' ORDER BY `helpchat`.id DESC LIMIT 200) t ORDER BY t.id ASC";
	$chats = $link -> query($query);
	while ($chat = mysqli_fetch_assoc($chats)) {
		$id = $chat['id'];
		$uuid = $chat['from'];
